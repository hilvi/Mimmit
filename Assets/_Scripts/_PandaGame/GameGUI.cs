...
public class GameGUI : MonoBehaviour {
	...
	void Start(){
		gameManager = GameObject.Find ("GameManager").GetComponent<PandaGameManager>();
	...
	void OnGUI() {
		if(GUI.Button (reloadPos.getRect(reloadTexture), reloadTexture)) {
		...
		if(enableSpeedup) {
			...
			if (GUI.Button(new Rect( - ( (int)( (1 - speedupLeft) * (speedupSpritesAmount - 1))) * flaskWidth, 0, buttonWidth,  buttonWidth), speedupSpriteSheet)) {
				speedupOn = !speedupOn;
				eraserOn = false;
		...
		if(enableEraser) {
			...
			if (GUI.Button(eraserPos.getRect(eraserTexture), eraserTexture)) {
					eraserOn = !eraserOn;
					speedupOn = false;
